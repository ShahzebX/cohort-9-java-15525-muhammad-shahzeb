...
public class AuthController {
    ...
    public AuthResponse login(@RequestBody LoginRequest loginRequest) {
        ...
        try {
            ...
            logger.warn("Login failed: unknown identifier: {}", loginRequest.getIdentifier());
            throw new InvalidCredentialsException("Invalid credentials");
        }

        if (!passwordEncoder.matches(loginRequest.getPassword(), user.getPasswordHash())) {
            logger.warn("Login failed: incorrect password for identifier: {}", loginRequest.getIdentifier());
            throw new InvalidCredentialsException("Invalid credentials");
        }

        logger.info("Login successful for user: {}", loginRequest.getIdentifier());
    ...
    public void changePassword(@RequestBody ChangePasswordRequest request, Authentication authentication) {
        ...
        logger.info("Password changed successfully for user: {}", identifier);
    }
}